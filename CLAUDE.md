...
- **`cache_manager.py`**: Parquet cache at `data/cache/market_monitor/prices.parquet` + `metadata.json`. Supports incremental refresh via `get_refresh_plan()` + `merge_incremental()` (combine_first pattern from ne73-dashboard). `is_stale()` checks metadata timestamp.
  - `incremental_eod_refresh(progress_callback=None) -> EodRefreshResult` is the shared implementation for the EOD refresh — pulls settled closes from RVX since `cached.index.max()`, merges via `merge_incremental`, saves. Used by **both** the offline script (`scripts/refresh_market_data.py --eod`) and the sidebar "EOD Refresh (RVX)" button so the two paths cannot drift. Returns a dataclass with `added_days`, `last_date`, `failed_tickers`, and a `skipped_reason` populated when no fetch happened (no cache, cache current, RVX returned empty).
- **Four-tier refresh**:
  1. **Bulk historical** (`python scripts/refresh_market_data.py --full`): One-time 10yr backfill from RVX. ~60s for 504 tickers.
  2. **EOD close — scheduled** (`python scripts/refresh_market_data.py --eod`): Daily incremental from RVX, scheduled trigger at 4:30 PM ET. Only fetches new settled closes. ~30s.
  3. **EOD close — on-demand** (sidebar "EOD Refresh (RVX)" button): Same `incremental_eod_refresh` as the script, callable from the UI. Useful for catching the cache up between scheduled runs (weekend backfill, mid-day catch-up, etc.). Renders a `st.progress` bar wired to the per-ticker fetch callback. After the merge, also appends a fresh `signal_history.parquet` snapshot so the Watchlist tab's deltas pick up the new EOD row.
  4. **Intraday live** (sidebar "Refresh Market Data" button): Uses yfinance for live prices during market hours. ~25s for 504 tickers. RVX `EQD(PRICE)` only provides settled daily closes (no intraday), so yfinance is used for live quotes.
